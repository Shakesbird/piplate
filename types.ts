...
// Starter recipes used large PNGs in earlier app versions. Keep existing local
// recipes intact while transparently serving their smaller replacement files.
export const getRecipeImageSource = (imageUri?: string) => {
  if (!imageUri) return DEFAULT_RECIPE_IMAGE;
  return imageUri.startsWith('./recipe-images/') && imageUri.endsWith('.png')
    ? `${imageUri.slice(0, -4)}.webp`
    : imageUri;
};
